...
(async function testSimplonline() {
    ...
    try {
        ...
        console.log("Navigation vers le menu des promotions");
        let promoMenu = await driver.wait(until.elementLocated(By.xpath("//input[@aria-label='Sélectionner une promotion']")), 10000);
        await promoMenu.click();
        console.log("Menu des promotions ouvert");

        await driver.sleep(1000);

        await promoMenu.sendKeys(Key.ARROW_DOWN);
        await promoMenu.sendKeys(Key.ARROW_DOWN);

        await promoMenu.sendKeys(Key.ENTER);
        ...
        await githubInput.sendKeys("https://github.com/TryZorce/SeleniumSandbox/tree/main/tests");
